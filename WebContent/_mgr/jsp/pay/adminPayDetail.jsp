...
function f_print(){
	...
	if($("#print_submission").is(":checked")){
		...
		var printNo = prompt("확인증 번호를 입력하세요", "숫자만 입력하세요");
...
<!-- Content.s -->
		<div id="content">
 			...
 			<p class="cont_header bold">
				<span class="f_left">회비납부/회원정보</span>
			</p>
							...
							<table class="form" style="width:600px">
								...
								<tr>
									...
									<td>
										...
										}else if("Y".equals(vo.getContact_status())){
											str_contact_status = "발송";
...
<%-- 								<tr>
							...
							<div class="mt10">
								...
								<span style="float:left">
									...
									<a href="javascript: f_print()" class="btn grey f_right">인쇄</a>&nbsp;&nbsp;
								</span>
								<br style="clear:both"/>
							</div>						
				...
				<div style="float:right; width:400px;">
							...
							<div class="list_wrap" id="list1">
								<table class="list">
									...
									%>
									<tr>
										<td class="first ta_center" style="width:80px"><%=d_pay.getString("gubun") %></td>
										<td class="ta_center" style="width:80px"><%=d_pay.getString("paydate") %></td>
										<td class="ta_center" style="width:50px"><%=d_pay.getString("pay") %></td>
										<td class="ta_right"><%=d_pay.getString("price") %></td>
										<td class="last ta_center" style="width:80px">
											<a href="javascript: f_selprice('<%=d_pay.getString("uid") %>')" class="btn grey">선택</a>
											<input type="hidden" id="i_uid_<%=d_pay.getString("uid") %>" value="<%=d_pay.getString("uid") %>"/>
											<input type="hidden" id="i_gubun_<%=d_pay.getString("uid") %>" value="<%=d_pay.getString("gubun") %>"/>
											<input type="hidden" id="i_paydate_<%=d_pay.getString("uid") %>" value="<%=d_pay.getString("paydate") %>"/>
											<input type="hidden" id="i_pay_<%=d_pay.getString("uid") %>" value="<%=d_pay.getString("pay") %>"/>
											<input type="hidden" id="i_price_<%=d_pay.getString("uid") %>" value="<%=d_pay.getString("price") %>"/>
										</td>
									</tr>
									<%
									}
