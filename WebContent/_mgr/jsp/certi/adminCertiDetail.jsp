...
function f_moveUpdatePage(){
	location.href = "adminCerti.do?selAction=update&uid=<%=qury_uid%>";
}
</script>
...
<div class="resultArea">
   ...
   <table class="table01 bgsky" border="1">
      <tr>
         ...
         <td>·접수번호 : <%=qury_regnum.replaceAll("\\(.*?\\)", "")%></td>
         <td>·접 수 일 : <%=qury_recv_date %></td>
      </tr>
      <tr>
         <td>·시험항목 : <%=qury_rqst_prdt_name %></td>
         <td>·봉인번호 : <%=qury_bongin %></td>
         <td>·완 료 일 : <%=qury_test_endd_date%></td>
      </tr>
      <tr>
         <td colspan="2">·검체표시 :<%=qury_GUMCHEPYOSI%></td>
   ...
   <table class="table01" border="1">
	...
	<%}else{ %>
      ...
      <tr class="titlecell">
	      	...
	      	<td>
	           	...
	           	<%}else{
					...
					<%	}%>
	           		...
	           		<%=format.format(Double.parseDouble(qury_NITRO)) %>
	           		<% }catch(Exception e){}
				} %>
	       	</td>
		</tr>
       		...
       		<td>14</td>
	           	...
	           	<%}else{
					...
					%>
						...
						불검출
					<%	}%>
					<br/><%=format.format(Double.parseDouble(qury_OXITE)) %>
	           	<% 	}catch(Exception e){}
				} %>
	       	</td>
		</tr>       	
       		...
       		<td>15</td>
	           	...
	           	<%}else{
					...
					<%			
						...
						<br/><%=format.format(Double.parseDouble(qury_QUMAPOS)) %>
	           		<% }catch(Exception e){}
				} %>
	       	</td>
		</tr>
			...
			<td>16</td>
				...
				<td>
					...
					<%			
								...
								<br/><%=format.format(Double.parseDouble(qury_SIMIAOZOL)) %>
					        	<%	}catch(Exception e){}
					} %>
			</td>
	  	...
	  	<%}else{
	 		try{
				...
				<%}else{%>
					불검출
			<%	}%>
			<br/><%=format.format(Double.parseDouble(qury_TETRASA)) %>
	     <%	}catch(Exception e){}
		} %>
			</td>
		</tr>
		...
		<%			
					...
					<br/><%=format.format(Double.parseDouble(qury_AMITRAZ)) %>				
		        <%		}catch(Exception e){}
		} %>
			</td>
		</tr>
		...
		<tr>
			...
			<%			
					...
					<br/><%=format.format(Double.parseDouble(qury_STREPTO)) %>
			        	<%	}catch(Exception e){}
			} %>
			</td>
		...
		<td>20</td>
			...
			<td>
				...
				<%			
						...
						<br/><%=format.format(Double.parseDouble(qury_BROMOP)) %>
				        	<%	}catch(Exception e){}
				} %>
			</td>
		...
		<%			
				...
				<br/><%=format.format(Double.parseDouble(qury_PLUBA)) %>
		        	<%	}catch(Exception e){}
		} %>
			</td>
		</tr>
		...
		<td>
			...
			<%			
					...
					<br/><%=format.format(Double.parseDouble(qury_GRAYANO)) %>
			        	<%	}catch(Exception e){}
			} %>
		</td>
		</tr>
		...
		<tr>
			...
			<td style="text-;"> 클로람페니콜</td>
				...
				<%			
						...
						</br><%=format.format(Double.parseDouble(qury_CRO)) %>
				        	<%	}catch(Exception e){}
				} %>
		</td>
		...
		<%			
				...
				<br/><%=format.format(Double.parseDouble(qury_PLUME)) %>
		        	<%	}catch(Exception e){}
		} %>
			</td>
		</tr>
		...
		<%			
				...
				<br/><%=format.format(Double.parseDouble(qury_NEOMA)) %>
		        	<%	}catch(Exception e){}
		} %>
			</td>
		</tr>
	...
	<div style="text-align: right;">
		...
		<span style="float:right">
			<a href="javascript: f_moveUpdatePage()" class="btn grey f_left">수정</a>
