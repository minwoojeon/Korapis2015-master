    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		oAjax.header("encoding","multipart/form-data");
	...
	function onAjaxError (){
     ...
     function uploadImage (e){
    	 if(true/*!bSupportDragAndDropAPI*/){
    		 generalUpload();
    	 }else{
    		 html5Upload();
    	 }
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	...
	window.onload = function(){
  		...
  		if(false/*bSupportDragAndDropAPI*/){
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			oFileInfo[0].sFileName = decodeURI(oFileInfo[0].sFileName);
			oFileInfo[0].sFileURL = decodeURI(oFileInfo[0].sFileURL);
