...
public class BbsDao2 {
	...
	public void makeWhere(MiceParam param){
		sqlWhere.setLength(0);
		if(!"".equals(param.getString("stx"))){
			if("wr_subject".equals(param.getString("sfl"))){
				sqlWhere.append("\n	AND bbs_title LIKE '%").append(param.getString("stx").replaceAll("'", "\\'")).append("%'		");
			}else if("wr_content".equals(param.getString("sfl"))){
				sqlWhere.append("\n	AND bbs_content LIKE '%").append(param.getString("stx").replaceAll("'", "\\'")).append("%'	");
			}else if("wr_subject||wr_content".equals(param.getString("sfl"))){
				sqlWhere.append("\n	AND ( 		");
				sqlWhere.append("\n		bbs_title LIKE '%").append(param.getString("stx").replaceAll("'", "\\'")).append("%'	");
				sqlWhere.append("\n		OR bbs_content LIKE '%").append(param.getString("stx").replaceAll("'", "\\'")).append("%'		");
				sqlWhere.append("\n	)		");
			}else if("mb_id,1".equals(param.getString("sfl")) || "mb_id,0".equals(param.getString("sfl"))){
				sqlWhere.append("\n	AND user_id = '%").append(param.getString("stxt")).append("%'		");
			}else if("wr_name,1".equals(param.getString("sfl")) || "wr_name,0".equals(param.getString("sfl"))){
				sqlWhere.append("\n	AND user_name = '%").append(param.getString("stxt")).append("%'		");
			}
		}
	}
	...
	public ResultSetEntity getBbsList(MiceParam param, int from, int cnt){
		...
		sql.append(sqlWhere);
		if("08".equals(param.getString("bflag"))){
			if(!"".equals(param.getString("PYEAR"))){
				sql.append("\n		AND a.bbs_pyear = '"+param.getString("PYEAR")+"'		");
			}
		}
	...
	public int getBbsList_tot(MiceParam param){
		...
		return TextFormatter.convNvl(dbh.getAData(sql.toString(), obj), 0);
	}
